...
import { ArrowLeft, ArrowRight, Check, Compass, Ship, Sparkles, Utensils, Car, LoaderCircle } from 'lucide-react';
...
export default function TripPlannerWizard() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (step !== 4 || transitioning || submitting) return;
    ...
    <section id="trip-planner" className="paper-section section-space border-t border-[#0b1513]/15" aria-labelledby="planner-title">
      <div className="container-wide">
        <div className="grid gap-12 lg:grid-cols-12">
          <Reveal variant="editorial" className="lg:col-span-4"><p className="eyebrow">04 · Bespoke planner</p><h2 id="planner-title" className="display title-md mt-7"><TextLines lines={['Yolculuğunuzu', <em key="second" className="text-[#80673f]">tasarlayın.</em>]} /></h2><p className="mt-7 max-w-sm text-sm leading-7 text-[#0b1513]/62">Tercihlerinizi seçin, ilk bütçe çerçevesini görün. Son program concierge ekibimizle birlikte netleşir.</p><p className="mt-8 border-l border-[#80673f] pl-4 text-xs leading-6 text-[#0b1513]/50">Gösterilen tutarlar tahmini başlangıç bedelleridir; tarih ve müsaitliğe göre değişebilir.</p></Reveal>
          ...
          <Reveal delay={.12} direction="right" className="lg:col-span-8">
            <div className="border border-[#0b1513]/20 bg-[#ebe5d9] p-5 sm:p-9">
              <ol className="grid grid-cols-4 border-b border-[#0b1513]/18 pb-6" aria-label={`Adım ${step} / 4`}>
                {STEP_LABELS.map((label, index) => <li key={label} aria-current={index + 1 === step ? 'step' : undefined} className={`planner-step ${index + 1 <= step ? 'text-[#0b1513]' : 'text-[#0b1513]/30'} flex flex-col gap-2`}><span className="font-serif text-2xl">0{index + 1}</span><span className="hidden text-[.58rem] font-bold uppercase tracking-[.14em] sm:block">{label}</span><motion.span className="planner-step-line" initial={false} animate={{ scaleX: index + 1 <= step ? 1 : 0 }} transition={{ duration: duration('medium'), ease: MOTION.ease }} aria-hidden="true" /></li>)}
              </ol>

              {success ? <div role="status" className="form-status-enter flex min-h-[30rem] flex-col items-center justify-center text-center"><span className="success-mark grid size-16 place-items-center border border-[#80673f] text-[#80673f]"><Check className="size-6" /></span><h3 className="display mt-7 text-5xl">Planınız bize ulaştı.</h3><p className="mt-4 max-w-md text-sm leading-7 text-[#0b1513]/60">Concierge ekibimiz seçiminizi inceleyerek sizinle iletişime geçecek.</p><p className="mt-6 font-serif text-2xl text-[#80673f]">{formatCurrency(total)}</p></div> :
              <form onSubmit={submit} className="pt-8 planner-form">
                <AnimatePresence mode="wait" initial={false}>
                  <motion.div key={step} initial="enter" animate="visible" exit="exit" variants={{ enter: { opacity: 0, x: reduced ? 0 : MOTION.distance.step }, visible: { opacity: 1, x: 0 }, exit: { opacity: 0, x: reduced ? 0 : -MOTION.distance.step } }} onAnimationComplete={phase => { if (phase === 'visible') setTransitioning(false); }} transition={{ duration: duration('medium'), ease: MOTION.ease }} className="min-h-[23rem]">
                    ...
                    {step === 4 ? <div><h3 className="font-serif text-3xl">Planı nereye gönderelim?</h3><div className="mt-8 grid gap-7 sm:grid-cols-2"><label><span className="block text-[.62rem] font-bold uppercase tracking-[.14em] text-[#80673f]">Adınız soyadınız *</span><input required autoComplete="name" className="mt-3 w-full border-0 border-b border-[#0b1513]/25 bg-transparent py-3 outline-none" value={data.name} onChange={e => setData({ ...data, name: e.target.value })} /></label><label><span className="block text-[.62rem] font-bold uppercase tracking-[.14em] text-[#80673f]">Telefon *</span><input required type="tel" autoComplete="tel" placeholder={CONTACT.phone} className="mt-3 w-full border-0 border-b border-[#0b1513]/25 bg-transparent py-3 outline-none" value={data.phone} onChange={e => setData({ ...data, phone: e.target.value })} /></label><label className="sm:col-span-2"><span className="block text-[.62rem] font-bold uppercase tracking-[.14em] text-[#80673f]">Özel notlar</span><textarea className="mt-3 min-h-24 w-full resize-y border-0 border-b border-[#0b1513]/25 bg-transparent py-3 outline-none" value={data.specialRequests} onChange={e => setData({ ...data, specialRequests: e.target.value })} /></label><div className="flex items-baseline justify-between border-t border-[#0b1513]/18 pt-5 sm:col-span-2"><span className="text-xs uppercase tracking-[.12em] text-[#0b1513]/50">Tahmini başlangıç</span><strong className="font-serif text-3xl text-[#80673f]">{formatCurrency(total)}</strong></div>{error ? <p role="alert" className="form-status-enter text-sm text-red-700 sm:col-span-2">{error}</p> : null}</div></div> : null}
                  </motion.div>
                </AnimatePresence>
                <div className="mt-6 flex items-center justify-between border-t border-[#0b1513]/18 pt-6">{step > 1 ? <button type="button" className="inline-flex min-h-11 items-center gap-2 text-[.65rem] font-bold uppercase tracking-[.13em]" disabled={transitioning} onClick={() => changeStep(step - 1)}><ArrowLeft className="size-4" /> Geri</button> : <span />}{step < 4 ? <button key="continue" type="button" disabled={transitioning} className="inline-flex min-h-11 items-center gap-2 bg-[#0b1513] px-5 text-[.65rem] font-bold uppercase tracking-[.13em] text-[#f4f0e7]" onClick={() => changeStep(step + 1)}>Devam <ArrowRight className="size-4" /></button> : <button key="submit" type="submit" disabled={submitting || transitioning} aria-busy={submitting} className="inline-flex min-h-11 items-center gap-2 bg-[#0b1513] px-5 text-[.65rem] font-bold uppercase tracking-[.13em] text-[#f4f0e7] disabled:opacity-50">{submitting ? 'İletiliyor…' : 'Planı gönder'} {submitting ? <LoaderCircle className="loading-indicator size-4" aria-hidden="true" /> : <ArrowRight className="size-4" />}</button>}</div>
              </form>}
            </div>
          </Reveal>
        </div>
